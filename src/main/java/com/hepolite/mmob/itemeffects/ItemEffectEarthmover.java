...
public class ItemEffectEarthmover extends ItemEffect
{
	...
	public void onRightClick(PlayerInteractEvent event, Player player, ItemStack item)
	{
		...
		if (block.getType() != Material.DIRT && block.getType() != Material.GRASS && block.getType() != Material.MYCELIUM && block.getType() != Material.GRAVEL && block.getType() != Material.SAND && block.getType() != Material.SNOW && block.getType() != Material.SNOW_BLOCK)
			return;
		...
		for (int x = -1; x <= 1; x++)
			for (int z = -1; z <= 1; z++)
				for (int y = -1; y <= 1; y++)
				{
					...
					if (newBlock.getType() != Material.DIRT && newBlock.getType() != Material.GRASS && newBlock.getType() != Material.MYCELIUM && newBlock.getType() != Material.GRAVEL && newBlock.getType() != Material.SAND && newBlock.getType() != Material.SNOW && newBlock.getType() != Material.SNOW_BLOCK)
						continue;
